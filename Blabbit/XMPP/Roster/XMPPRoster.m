...
/**
 * Blabbit's version of XMPPRoster.m that ensures the main thread is always
 * notified when there is a roster push or after initial population of roster.
 * This is required to immediately inform users of changes to contacts.
 *
 * This can be approached in two ways:
 * 1. At the end of -[XMPPRoster _addRosterItems:] call 
 *    -[XMPPRosterCoreDataStorage saveAndPostNotification]. 
 *    This requires importing XMPPRosterCoreDataStorage.h but is the cleaner solution
 *
 * 2. BBTXMPPManager would implement delegate methods xmppRoster:didReceiveRosterPush:
 *    and xmppRosterDidEndPopulating: where it would call saveAndPostNotification.
 *    This only requires updating xmppStream:didReceiveIQ: (in this file) to
 *    call the multicast delegate roster push method after calling _addRosterItems:
 *
 * Chosen approach:
 * I choose to go with solution 1. Having dependencies on order of method calls
 * (as in solution 2) is just horrid.
 */
...
- (void)_addRosterItems:(NSArray *)rosterItems
{
    ...
    // See file header for why we have to save roster and propagate changes to
    // main thread's managed object context
    if ([xmppRosterStorage isKindOfClass:[XMPPRosterCoreDataStorage class]]) {
        [((XMPPRosterCoreDataStorage *)xmppRosterStorage) saveAndPostNotification];
    }
}
